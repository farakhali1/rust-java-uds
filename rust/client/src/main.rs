...
fn main() {
    ...
    let n = 1005000;
    let mut i = 0;
    let mut vec: Vec<u128> = Vec::with_capacity(n);
    ...
    match UnixStream::connect(&socket_path) {
        Ok(mut stream) => {
            ...
            if args[1] == "1" {
                    ...
                    match stream.read(&mut response) {
                        ...
                        Ok(n) if n > 0 => {
                            let response_message =
                                Result::parse_from_bytes(&response[..n]).unwrap();
                ...
                panic!("Invalid argument, CLI option must be 1 or 2");
            }
        }
        Err(err) => {
